...
write_gateway_env_file() {
    local target_dir="$HOME/.config/openclaw"
    local target_file="$target_dir/gateway.env"
    mkdir -p "$target_dir"
    umask 077
    {
        echo "# Generated by profit-corp/setup_corp.sh"
        ...
        [[ -n "${OPENCLAW_HOOKS_TOKEN:-}" ]] && printf 'OPENCLAW_HOOKS_TOKEN=%s\n' "$OPENCLAW_HOOKS_TOKEN"
        [[ -n "${CORP_TIMEZONE:-}" ]] && printf 'CORP_TIMEZONE=%s\n' "$CORP_TIMEZONE"
    } > "$target_file"
    info "Synced runtime env to $target_file"
}
...
ensure_required_runtime_env() {
    local env_file="$1"
    if ! has_real_value "${OPENCLAW_HOOKS_TOKEN:-}"; then
        local generated_hooks_token
        generated_hooks_token="$(generate_random_hex 32)"
        upsert_env_var "$env_file" "OPENCLAW_HOOKS_TOKEN" "$generated_hooks_token"
        export OPENCLAW_HOOKS_TOKEN="$generated_hooks_token"
        info "Generated OPENCLAW_HOOKS_TOKEN and saved it to .env"
    fi

    if ! has_real_value "${TELEGRAM_BOT_TOKEN:-}"; then
        warn "TELEGRAM_BOT_TOKEN is missing or still set to the placeholder value."
    fi
}
...
load_env_file "$ENV_FILE"
ensure_required_runtime_env "$ENV_FILE"
load_env_file "$ENV_FILE"
write_gateway_env_file
collect_model_configuration
